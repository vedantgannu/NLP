...
def get_training_instances(dep_structure):
    ...
    sorted_nodes = [k for k,v in sorted(deprels.items())]#sorted word ids from a sentence
...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        stack, buffer = list(reversed(state.stack[-3:])), list(reversed(state.buffer[-3:]))
        #Stack and buffer contain indices of words specific to a sentence
        #Need to map these indices to words, and then words to indices within words_vocab
        stack_indices = []
        buffer_indices = []
        for word_index in stack:
            if words[word_index] is None:#<ROOT> case
                stack_indices.append(self.word_vocab["<ROOT>"])
            elif pos[word_index] == "CD":#<CD> case
                stack_indices.append(self.word_vocab["<CD>"])
            elif pos[word_index] == "NNP":#<NNP> case
                stack_indices.append(self.word_vocab["<NNP>"])
            else:
                vocab_index = self.word_vocab.get(words[word_index].lower(), None)
                if vocab_index is not None:#If the word exists
                    stack_indices.append(vocab_index)
                else:#Unknown word case (word was only seen once across the entire corpus)
                    stack_indices.append(self.word_vocab["<UNK>"])
        stack_indices += ( (3 - len(stack_indices)) * [self.word_vocab["<NULL>"]] )
        
        for word_index in buffer:
            if words[word_index] is None:#<ROOT> case
                buffer_indices.append(self.word_vocab["<ROOT>"])
            elif pos[word_index] == "CD":#<CD> case
                buffer_indices.append(self.word_vocab["<CD>"])
            elif pos[word_index] == "NNP":#<NNP> case
                buffer_indices.append(self.word_vocab["<NNP>"])
            else:
                vocab_index = self.word_vocab.get(words[word_index].lower(), None)
                if vocab_index is not None:#If the word exists
                    buffer_indices.append(vocab_index)
                else:#Unknown word case (word was only seen once across the entire corpus)
                    buffer_indices.append(self.word_vocab["<UNK>"])
        buffer_indices += ( (3 - len(buffer_indices)) * [self.word_vocab["<NULL>"]] )
        #print("Snapshot", [self.word_vocab[_id] for _id in stack_indices + buffer_indices])
        #print("np arr", np.array(stack_indices + buffer_indices))
        
        return np.array(stack_indices + buffer_indices)

    def get_output_representation(self, output_pair):
        # TODO: Write this method for Part 2
        #Given (transition string, dependency relation string) create 91 length 1 hot encoded output vector
