...
def build_model(word_types, pos_types, outputs):
    ...
    model.add(Flatten())#Flatten the output of the embedding layer since we get batch_sizex6x32 dim objects
    model.add(Dense(units=100, activation='relu'))
    model.add(Dense(units=10, activation='relu'))
    model.add(Dense(units=outputs, activation=keras.activations.softmax))
